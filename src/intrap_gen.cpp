...
void generateCodeForProcedure(std::vector<BB> basicBlocks, z3::model &model, z3::context &ctx, int sample_number, std::string uuid)
{
    ...
    outputFile << "}\n";
    // outputFile << "int main() {\n";
    // //make a call to the function with the initial values extracted from the model
    // std::vector<int> initialisations = extractInitialisationsFromModel(model, ctx);
    // outputFile << "    function_" << uuid << "_"<< std::to_string(sample_number) << "(";
    // for (int i = 0; i < NUM_VARS; ++i) {
    //     outputFile << initialisations[i];
    //     if (i < NUM_VARS - 1) {
    //         outputFile << ", ";
    //     }
    // }
...
int main(int argc, char** argv)
{
    ...
    mappingFile = std::ofstream("mappings/mapping_" + std::to_string(sample_number) + ".txt");
    outputFile = std::ofstream("procedures/function_" + uuid + "_" + std::to_string(sample_number) + ".c");
    ...
    {
        //logFile <<"UNSAT\n";
        //remove the output file and mapping file from the filesystem
        outputFile.close();
        mappingFile.close();
        std::remove(("procedures/function_" + uuid + "_" + std::to_string(sample_number) + ".c").c_str());
        std::remove(("mappings/mapping_" + std::to_string(sample_number) + ".txt").c_str());
        // logFile.close();
        return 1;
    }
    else
    ...
    //dump query and model to log file
    //now, let's try to generate a couple more mappings. First, let's regenerate the SMT query
    solver.reset();
    solver.add(makeInitialisationsInteresting(c));
    if(enableRandomInitialisations)
    {
    ...
    {
        solver.add(differentInitialisationConstraint(initialisation, c));
        if(solver.check() == z3::unsat)
        {
            //logFile <<"UNSAT\n";
            outputFile.close();
