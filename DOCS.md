...
The above is the default mode called `normal`, i.e., this mode collects well-definedness constraints to ensure the entire execution path is free of UB. There is also a `ryubs` mode, where it intentionally negates the well-definedness constraint at one selected statement on the execution path to guarantee that at least one specific UB is triggered after constraint solving.
