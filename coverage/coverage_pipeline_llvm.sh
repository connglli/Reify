#!/bin/bash
# =============================================================================
# coverage_pipeline_llvm.sh
# Full LLVM/Clang coverage pipeline: clean, parallel profile, merge, lcov, genhtml.
#
# Usage:
#   ./coverage_pipeline_llvm.sh [OPTIONS] TOOL_DIR [TOOL_DIR ...]
#
# Each TOOL_DIR is specified as:   <type>:<path>
# Supported types:
#   rysmith   — recursively finds func.c files
#   rylink    — finds *.c files inside prog_* subdirs
#   csmith    — finds *.c files at top level, adds csmith include
#   yarpgen   — finds driver.c + func.c inside yarpgen_sample_* subdirs
#
# Options:
#   --clang PATH        Path to instrumented Clang (default: /reify/coverage/builds/llvm-instrumented/bin/clang)
#   --llvm-build PATH   Path to LLVM build dir with .gcno files (default: /reify/coverage/builds/llvm-build)
#   --results PATH      Results output dir (default: /reify/coverage/results_llvm)
...
# DEFAULTS
# =============================================================================
CLANG="/reify/coverage/builds/llvm-instrumented/bin/clang"
LLVM_BUILD="/reify/coverage/builds/llvm-build"
RESULTS_DIR="/reify/coverage/results_llvm"
...
INFO_FILE="$RESULTS_DIR/llvm_coverage.info"
REPORT_DIR="$RESULTS_DIR/llvm_report"
LIBGCOVFLUSH="/reify/coverage/builds/libgcovflush.so"
mkdir -p $REPORT_DIR
PROCS=$(nproc)
GCOV_STRIP=$(echo "$LLVM_BUILD" | tr '/' '\n' | grep -c .)
...
# =============================================================================
# Use the gcov/gcov-tool that matches the compiler used to build the instrumented LLVM.
# LLVM was compiled by gcc-11, so derive gcov/gcov-tool from gcc-11's install.
# Same pattern as coverage_pipeline_gcc.sh.
HOST_GCC="$(command -v gcc-11 2>/dev/null || command -v gcc)"
HOST_GCC_INSTALL="$(dirname "$(dirname "$HOST_GCC")")"
...
# STEP 1 — clean
# =============================================================================
info "Step 1: Cleaning old .gcda files from $LLVM_BUILD..."
find "$LLVM_BUILD" -name "*.gcda" -delete

REMAINING=$(find "$LLVM_BUILD" -name "*.gcda" | wc -l)
            ...
            case "$TOOL" in
                CSMITH_FILE)
                    ...
                    LD_PRELOAD="$LIBGCOVFLUSH" timeout "$TIMEOUT_SECS" \
                        "$CLANG" -O3 -I"$CSMITH_INCLUDE" -c "$f" -o /dev/null \
                        2>/dev/null || true
                    ;;
                RYSMITH)
                    cd "$UNIT"
                    LD_PRELOAD="$LIBGCOVFLUSH" timeout "$TIMEOUT_SECS" \
                        "$CLANG" -O3 -c func.c -o /dev/null \
                        2>/dev/null || true
                    ;;
                RYLINK)
                    cd "$UNIT"
...
info "Copying merged .gcda files into $LLVM_BUILD..."
REMAINING=$(find "$LLVM_BUILD" -name "*.gcda" | wc -l)
if [ "$REMAINING" -ne 0 ]; then
    die "$REMAINING .gcda files still present in $LLVM_BUILD before copy — aborting."
fi
cp -r "$MERGED_DIR/." "$LLVM_BUILD/"
...
lcov --capture \
	...
	> /dev/null 2>&1
# Filter to llvm/ and clang/ source only — exclude generated files,
# third-party libs, system headers, and other llvm-project subdirs
info "Step 5b: Filtering to llvm/ and clang/ sources..."
