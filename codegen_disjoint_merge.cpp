        ...
        z3::expr generateConstraints(int target,  z3::solver& solver, z3::context& c, std::unordered_map<std::string, int>& versions, int walk_init_index)
        {
            ...
            if(blockTargets.size() > 1){
                ...
                for(auto i: conditionalVariables)
                {
                    ...
                    {
                        logFile<<"pushing back "<<generateConditionalCoefficientName(blockno, 0, ctr)<<" "<<parameters[generateConditionalCoefficientName(blockno, 0, ctr)].value()<<'\n';
                        coeffs.push_back(c.int_const((generateConditionalCoefficientName(blockno, 0, ctr)).c_str()));
                        parameters[generateConditionalCoefficientName(blockno, 0, ctr)] = std::nullopt;
                    }
                    // coeffs.push_back(c.int_const((generateConditionalCoefficientName(blockno, 0, ctr)).c_str()));
...
void extractParametersFromModel(z3::model &model, z3::context &ctx)
{
    for (const auto& param : parameters) {
        ...
        else if(parameters[name].has_value())
        {
            continue;
        }
        else {
            ...
            logFile << "Parameter " << name << " is not in the model, assigning random value: " 
                      << randomValue << '\n';
        }
    }
...
void dumpCodeToFile(std::vector<BB> basicBlocks, const std::string& filename, const std::string& code, const std::vector<std::vector<int>> &initialisations) 
{
    ...
    outputFile << "}\n";
    //now let's write all the pass counters
    for(int i = 0; i < NODES; i++)
    {
            outputFile << "int " << generatePassCounterName(i) << " = 0;\n";
    }
    outputFile << "int main() {\n";
...
z3::expr generateDifferentInitialisationConstraints(int walk_ctr, int initialisation_ctr_1, int initialisation_ctr_2, z3::context& c)
{
    ...
    z3::expr differentInitialisationConstraint = atMostKZeroes(c, differentInitialisationConstraints, NUM_VARS/2, 0);
...
int main(int argc, char** argv)
{
    ...
    walk_ctr = 0;
    assert(NUMBER_OF_DISTINCT_WALKS == 1);
    //generate one satisfying constraint for each BB, according to the walk taken
    for (auto& sample_walk : sample_walks){
    ...
    solver.add(addRandomInitialisations(0, c));
    logFile << "generated query for this walk is \n";
    logFile << solver.to_smt2() << std::endl;
    //now we need to add the constraints for the initialisation variables
    ...
    else
    {
        ...
        {   
            ...
            for(int j = 0; j < sample_walks[0].size() - 1; j++)
            {
                int current_bb = sample_walks[0][j];
                int next_bb = sample_walks[0][j + 1];
            ...
            this_walk_taken_constraint = this_walk_taken_constraint && basicBlocks[sample_walks[0][sample_walks[0].size() - 1]].generateConstraints(-1, solver, c, versions, walk_init_index); 
        ...
        {
            logFile << "Model is unsatisfiable\n";
            logFile << "Query:\n" << solver.assertions() << std::endl;
            exit(1);
        }
    }
