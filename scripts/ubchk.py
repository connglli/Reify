...
def _check_ubs(c_file, o_file='/tmp/x.out', cmp_tmo=60, exe_tmo=120):
    try:
        run_proc(
            [
                'gcc',
...
def check_ubs(func_path, map_path):
    func_path, map_path = Path(func_path), Path(map_path)
    func_code, func_name = func_path.read_text(), func_path.stem
    tmp_c_path, tmp_o_path = Path('/tmp/x.c'), Path('/tmp/x.out')
