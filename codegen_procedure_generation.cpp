...
void pushAfterSearch(std::vector<z3::expr> &coeffs, std::string name, z3::context &ctx){ 
    ...
    // otherwise it's an uninterpreted constant which the solver needs to figure out the value for
    if(parameters.find(name) != parameters.end() && parameters[name].has_value())
    {
        coeffs.push_back(ctx.int_val(parameters[name].value()));
    }
    else if (parameters.find(name) == parameters.end() || (parameters.find(name) != parameters.end() && parameters[name] == std::nullopt))
    {
...
class BB{
    ...
    public:
        BB(BasicBlockNo blockno, const std::set<BasicBlockNo>& graphTargets) : blockno(blockno), needPassCounter(false)
        {
            ...
            {
                statementMappings[varIndex] = sampleKDistinct(NUM_VARS, VARIABLES_PER_ASSIGNMENT_STATEMENT); //sample the variables which will be used in the RHS of the assignment statement
                ...
                {
                    std::string coefficientKey =  getCoefficientName(blockno, statementIndex, i); //statement index is the index of the assignment statement in a basic block, and i is the index of the variable in the RHS of the assignment statement
            ...
            if(blockTargets.size() > 1)
            {
                ...
                {
                    std::string coefficientKey = generateConditionalCoefficientName(blockno, 0, i); //0 is the statement index here (even I'm not sure why i chose this as a parameter but i don't want to break the code so let's just not touch this), and i is the index of the variable in the actual conditional
                    parameters[coefficientKey] = std::nullopt;
                }
                std::string constantKey = generateConditionalConstantName(blockno, blockTargets[0]); // target block is a parameter here if we ever wanted to support more that 2 potential targets for the same basic block
                parameters[constantKey] = std::nullopt;
            }
        }
        ...
        std::string generateCode() const
        {
            ...
            for (const auto& [varIndex, dependencies] : statementMappings)
            {
                ...
                for (size_t i = 0; i < dependencies.size(); ++i)
                {
                    ...
                    {
                        // dead code that the model didn't find an interpretation for so we can do whatever. assign a random value to these coefficients
                        coefficient = dist(gen);  
        ...
        void generateConstraints(int target,  z3::solver& solver, z3::context& c, std::unordered_map<std::string, int>& versions)
        {
            ...
            for (const auto& [varIndex, dependencies] : statementMappings)
            {
                ...
                //UB checks for RHS of the assignment statement. Each subexpression is a multiplication of a coefficient and a variable, and it should be between LOWER_BOUND and UPPER_BOUND
                ...
                for (int i = 1; i < terms.size(); i++) {
                    z3::expr constraint = (sum <= UPPER_BOUND) && (sum >= LOWER_BOUND); //addition is left associative, so in an addition of k terms, the subexpressions with the first 1, 2, ...k terms added should all be between LOWER_BOUND and UPPER_BOUND as well
                ...
                //unrolled the entire graph and tracking the versions of each variable because they change adter assignment statements
                //LHS of the assignment statement's version number is changed
...
// Function to extract parameters from the model, so that we can store the coefficients and constants that the solver found an interpretation for
...
// to extract the initial values of each variable that's an input to the function from the model
...
//i wanted a bit of flexibility for the checksum function in the future so i decided to just store the final values of all the variables (at the end basic block) instead of just the checksum
std::vector<int> extractFinalVariableVersions(z3::model &model, z3::context &ctx, std::unordered_map<std::string, int> &versions){
...
void generateCodeForProcedure(std::vector<BB> basicBlocks, z3::model &model, z3::context &ctx, int sample_number, std::string uuid)
{
...
void cleanupIfEmpty(uuid)
{
    ...
    std::remove(("procedures/function_" + std::to_string(getpid()) + ".c").c_str());
    std::remove(("mappings/function_" + std::to_string(getpid()) + "_mapping").c_str());
}
...
int main(int argc, char** argv)
{
    ...
    changeUUID(uuid); //used a UUID so that i could throw everything from different runs into the same directory without worrying about name clashes
    ...
    outputFile = std::ofstream(openFileName);
    ...
    {
        //this means that the last node is not the end node, probably because we're going in a cycle, so we need to add a pass counter to the last node (as if we're artificially creating an edge to the end node)
    ...
    for(int i = 0; i < sample_walk.size() - 1; i++)
    {
        ...
        basicBlocks[current_bb].generateConstraints(next_bb, solver, c, versions); //generate constraints for the current basic block so that no UB occurs and it can reach the next basic block
    }
    ...
    // First, let's regenerate the SMT query by repopulating the solver with the same constraints, 
    // except this time - the solver would use the coefficients and constants we already generated from the model
    solver.reset();
    solver.add(makeInitialisationsInteresting(c));
    if(enableRandomInitialisations)
    {
    ...
    {
        solver.add(differentInitialisationConstraint(initialisation, c)); // ensure that the initialisation is sufficiently different from the previous one 
        if(solver.check() == z3::unsat)
        {
            //logFile <<"UNSAT\n";
            outputFile.close();
